...
import {GrPolygon} from "../Objects/GrPolygon";
...
export class DrawPolygon extends Tool {
    ...
    public update(interactionEvent: InteractionEvents, eventData: InteractionEventData = null): boolean {
        ...
        switch (this._state.state.id) {
            ...
            case States.Wait:
                if (this._poly && interactionEvent === InteractionEvents.MouseMove) {
                    newp = new Point2D(eventData.x, eventData.y);
                    this._poly.setPoint(this._poly.points.length - 1, newp);
                    this._renderer.renderPolygon(RenderLayer.Interaction, this._poly);
                }
                break;
            ...
            case States.Point:
                if (interactionEvent == InteractionEvents.Click) {
                    ...
                    if (!this._poly) {
                        this._poly = GrPolygon.create(null, [newp, newp], false);
                    } else {
    ...
    public get result(): any {
        if (this._poly && this._poly.points.length > 1) {
            return `POLY $drawing ${this._poly.name} "${this._poly.name}" $styles.default [ ${this._poly.points.map(p => `(${p.x} ${p.y})`).join(" ")} ] ${this._closed ? 1 : 0}`;
        } else {
            return null;
        }
    }
